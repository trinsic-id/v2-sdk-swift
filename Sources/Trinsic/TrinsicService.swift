...
public class TrinsicService : ServiceBase {
    ...
    public func account() -> AccountService {
        _accountService = _accountService ?? AccountService(options: self.options)
        return _accountService!
    }
    
    public func credential() -> CredentialService {
        _credentialService = _credentialService ?? CredentialService(options: self.options)
        return _credentialService!
    }
    
    public func provider() -> ProviderService {
        _providerService = _providerService ?? ProviderService(options: self.options)
        return _providerService!
    }
    
    public func template() -> TemplateService {
        _templateService = _templateService ?? TemplateService(options: self.options)
        return _templateService!
    }
    
    public func trustRegistry() -> TrustRegistryService {
        _trustRegistryService = _trustRegistryService ?? TrustRegistryService(options: self.options)
        return _trustRegistryService!
    }
    
    public func wallet() -> WalletService {
        _walletService = _walletService ?? WalletService(options: self.options)
